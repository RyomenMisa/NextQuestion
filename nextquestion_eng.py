#!/usr/bin/env python3
# nextquestion_eng.py — English locale (logs, TUI wizard, note labels).
# Companion file: nextquestion_ru.py (Russian locale). Logic is identical.
"""nextquestion (English locale) — graph-augmented RAG pipeline:
documents → SQLite → Obsidian graph → streamed answers.
...
# Latin-only heading detector (English locale build).
HEADING_RE = re.compile(r"^\s*(?:\d+(?:\.\d+)*)?\s*[A-Z][A-Za-z0-9 ,\-/()]{3,70}$")
FTS_STOP_WORDS = {"this", "that", "with", "from", "which", "their", "about",
                  "against", "between", "through", "during", "and", "or",
                  "not", "near", "also", "into", "over", "than", "then",
                  "them", "these", "those", "there", "where", "when"}
SEED_LIMIT = 5
...
def normalize_text(value: str) -> str:
    """Lowercase and strip punctuation for entity/keyword matching."""
    if not value:
        return ""
    ...
    def extract_pages(self, path: Path, start: int | None, end: int | None):
        if path.suffix.lower() == ".pdf":
            ...
            try:
                ...
                doc.close()
        raw = path.read_bytes()
        # UTF-8 first; latin-1 as a universal fallback (it never fails).
        for enc in ("utf-8", "latin-1"):
            try:
                return [(1, raw.decode(enc).replace("\x00", ""))], [], 1
            except UnicodeDecodeError:
                continue
        raise SourceError(f"Could not decode text file: {path}")
    ...
    def segment(self, pages, toc, s_page: int, e_page: int, book: str) -> list[Chunk]:
        ...
        def flush(end_page: int) -> None:
            ...
            if text and cur_title:
                raw_chunks.append({
                    "title": cur_title,
                    "chapter": cur_chapter or "Untitled chapter",
                    "text": text,
...
class Retriever:
    ...
    def seeds(self, exp: QueryExpansion, limit: int = SEED_LIMIT) -> list:
        ...
        # Latin-only tokenization (English locale build).
        words = re.findall(r"\b[A-Za-z]{4,}\b", " ".join(exp.hyde))
...
def print_banner(cfg: AppConfig, store: Store) -> None:
    ...
    info = [
        f"{W}NEXTQUESTION{R} {D}v1.0.0 (English locale){R}",
        f"{D}──────────────────────────────────────────────{R}",
        f"{W}Engine{R}     : Graph-Augmented RAG Pipeline",
...
def init_wizard(cfg_path: Path) -> None:
    ...
    print("   Next: run 'nextquestion_eng.py chunk <document.pdf>' to start building your knowledge base.\n")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="nextquestion",
        description="NextQuestion (English locale) — graph-augmented RAG: documents → Obsidian → answers."
    )
...
def main(argv: list[str] | None = None) -> None:
    ...
    if not args.config.exists():
        ...
        LOG.info("💡 Run 'python nextquestion_eng.py init' for interactive setup, "
                 "or 'python nextquestion_eng.py example' to see the template.")
        sys.exit(1)
